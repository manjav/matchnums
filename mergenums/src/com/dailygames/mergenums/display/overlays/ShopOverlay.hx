...
class ShopOverlay extends ConfirmPopup {
	...
	override private function initialize():Void {
		...
		IAP.addEventListener(IAPEvent.PURCHASE_INIT, IAP_purchaseInitHandler);
		// IAP.addEventListener(IAPEvent.PURCHASE_INIT_FAILED, IAP_purchaseInitHandler);

		IAP.initialize("");
	...
	private function listView_changeHandler(event:Event):Void {
		trace(this.listView.selectedIndex);
		...
		if (text == "Ads Free") {
			...
			IAP.addEventListener(IAPEvent.PURCHASE_SUCCESS, IAP_purchaseSuccessHandler);
			IAP.purchase("item_" + this.listView.selectedIndex);
		}
		this.listView.selectedIndex = -1;
		this.listView.addEventListener(Event.CHANGE, this.listView_changeHandler);
	}

	private function IAP_purchaseInitHandler(event:IAPEvent):Void {
		trace(event);
		...
		function onProductsDataComplete(e:IAPEvent) {
			...
			for (data in e.productsData) {
				...
				trace(data.productID, data.price);
			}
		}
		...
		var productIds:Array<String> = ["item_0", "item_1"];
		IAP.requestProductData(productIds);
	}

	private function IAP_purchaseSuccessHandler(event:IAPEvent):Void {
		IAP.removeEventListener(IAPEvent.PURCHASE_SUCCESS, IAP_purchaseSuccessHandler);
		IAP.addEventListener(IAPEvent.PURCHASE_CONSUME_SUCCESS, IAP_consumeSuccessHandler);
		IAP.consume(event.purchase);
	}

	private function IAP_consumeSuccessHandler(event:IAPEvent):Void {
		trace(event);
		IAP.removeEventListener(IAPEvent.PURCHASE_CONSUME_SUCCESS, IAP_consumeSuccessHandler);
